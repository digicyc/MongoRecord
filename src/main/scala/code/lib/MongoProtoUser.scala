...
trait ProtoUser[T <: ProtoUser[T]] extends MongoRecord[T] with UserIdAsString with MongoId[T]{
  ...
  lazy val mobile: StringField[T] = new MyMobile(this,15)

  protected class MyMobile(obj: T, size: Int) extends StringField(obj, size) {
    override def displayName = owner.mobileDisplayName
    override val fieldId = Some(Text("mobile"))
  }

  /**
   * The last name string
   */
  def mobileDisplayName = ??("mobile.no")

  lazy val country: StringField[T] = new MyCountry(this,15)

  protected class MyCountry(obj: T, size: Int) extends StringField(obj, size) {
    override def displayName = owner.countryDisplayName
    override val fieldId = Some(Text("country"))
  }

  /**
   * The last name string
   */
  def countryDisplayName = ??("country.name")

  lazy val city: StringField[T] = new MyCity(this,15)
  protected class MyCity(obj: T, size: Int) extends StringField(obj, size) {
    override def displayName = owner.cityDisplayName
    override val fieldId = Some(Text("city"))
  }
  def cityDisplayName = ??("city.name")
